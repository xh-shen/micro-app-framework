<!--
 * @Author: shen
 * @Date: 2022-05-29 16:37:40
 * @LastEditors: shen
 * @LastEditTime: 2022-05-29 17:10:59
 * @Description: 
-->
<script setup lang="ts">
import { useStore } from '@/hooks'
import { useI18n } from '@micro/hooks'
import { Message } from '@micro/utils'
import { computed } from 'vue'

const { state, dispatch } = useStore()
const { locale, t } = useI18n()

const lang = computed(() => state.app.lang)

const list = [
  {
    lang: 'zh-cn',
    label: '简体中文',
    icon: '🇨🇳',
  },
  {
    lang: 'en',
    label: 'English',
    icon: '🇺🇸',
  },
]

const onCommand = async (command: string) => {
  locale.value = command
  dispatch('app/setAppLang', command)
  Message('Switch Language Success')
}
</script>

<template>
  <ElDropdown @command="onCommand">
    <span class="mc-select-lang">
      <svg viewBox="0 0 24 24" focusable="false" width="1em" height="1em" fill="currentColor" aria-hidden="true">
        <path d="M0 0h24v24H0z" fill="none"></path>
        <path
          d="M12.87 15.07l-2.54-2.51.03-.03c1.74-1.94 2.98-4.17 3.71-6.53H17V4h-7V2H8v2H1v1.99h11.17C11.5 7.92 10.44 9.75 9 11.35 8.07 10.32 7.3 9.19 6.69 8h-2c.73 1.63 1.73 3.17 2.98 4.56l-5.09 5.02L4 19l5-5 3.11 3.11.76-2.04zM18.5 10h-2L12 22h2l1.12-3h4.75L21 22h2l-4.5-12zm-2.62 7l1.62-4.33L19.12 17h-3.24z "
          class="css-c4d79v"
        ></path>
      </svg>
      {{ t('button.confirm') }}
    </span>
    <template #dropdown>
      <ElDropdownMenu>
        <ElDropdownItem v-for="item in list" :key="item.lang" :disabled="item.lang === lang" :command="item.lang">{{ item.icon }} {{ item.label }}</ElDropdownItem>
      </ElDropdownMenu>
    </template>
  </ElDropdown>
</template>

<style lang="scss" scoped>
.mc-select-lang {
  display: flex;
  align-items: center;
  margin-left: 10px;
}
</style>
