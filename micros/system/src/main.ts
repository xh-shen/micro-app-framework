 ...
 * @LastEditors: shen
 * @LastEditTime: 2022-05-15 23:34:54
 * @Description:
 */
...
function handleMicroData(router: Router) {
  ...
  if (window.__MICRO_APP_ENVIRONMENT__) {
    // 主动获取基座下发的数据
    console.log('system getData:', window.microApp.getData())

    // 监听基座下发的数据变化
    window.microApp.addDataListener((data: Record<string, unknown>) => {
      console.log('system addDataListener:', data)
    ...
    // 向基座发送数据
    setTimeout(() => {
      window.microApp.dispatch({ myname: 'system' })
    }, 3000)
  }
}
...
function fixBugForVueRouter4(router: Router) {
  ...
  if (window.location.href.includes('/system')) {
    /**
     * 重要说明：
     * 1、这里主应用下发的基础路由为：`/app-vue3`，其中 `/main-xxx` 是主应用的基础路由，需要去掉，我们只取`/app-vue3`，不同项目根据实际情况调整
     *
