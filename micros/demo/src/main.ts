 ...
 * @LastEditors: shen
 * @LastEditTime: 2022-05-15 23:10:23
 * @Description:
 */
...
function handleMicroData(router: Router) {
  ...
  if (window.__MICRO_APP_ENVIRONMENT__) {
    // 主动获取基座下发的数据
    console.log('demo getData:', window.microApp.getData())

    // 监听基座下发的数据变化
    window.microApp.addDataListener((data: Record<string, unknown>) => {
      console.log('demo addDataListener:', data)
    ...
    // 向基座发送数据
    setTimeout(() => {
      window.microApp.dispatch({ myname: 'demo' })
    }, 3000)
  }
}
